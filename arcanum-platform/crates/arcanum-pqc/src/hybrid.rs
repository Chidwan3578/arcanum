...
const ML_KEM_768_DK_SIZE: usize = 2400;
const ML_KEM_768_EK_SIZE: usize = 1184;
...
impl X25519MlKem768DecapsulationKey {
    /// Get the X25519 public key component.
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(32 + ML_KEM_768_DK_SIZE);
        bytes.extend_from_slice(&self.x25519_secret.to_bytes());
        bytes.extend_from_slice(&self.ml_kem_dk);
        bytes
    }

    /// Import from bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != 32 + ML_KEM_768_DK_SIZE {
            return Err(Error::InvalidKeyLength {
                expected: 32 + ML_KEM_768_DK_SIZE,
                actual: bytes.len(),
            });
        }

        let x25519_bytes: [u8; 32] = bytes[..32].try_into().unwrap();
        let ml_kem_bytes = bytes[32..].to_vec();

        Ok(Self {
            x25519_secret: StaticSecret::from(x25519_bytes),
            ml_kem_dk: ml_kem_bytes,
        })
    }
}

impl Drop for X25519MlKem768DecapsulationKey {
    fn drop(&mut self) {
...
impl X25519MlKem768EncapsulationKey {
    /// Export to bytes.
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(32 + ML_KEM_768_EK_SIZE);
        bytes.extend_from_slice(self.x25519_public.as_bytes());
        bytes.extend_from_slice(&self.ml_kem_ek);
        bytes
    }

    /// Import from bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != 32 + ML_KEM_768_EK_SIZE {
            return Err(Error::InvalidKeyLength {
                expected: 32 + ML_KEM_768_EK_SIZE,
                actual: bytes.len(),
            });
        }
...
impl X25519MlKem768Ciphertext {
    /// Export to bytes.
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::with_capacity(32 + ML_KEM_768_CT_SIZE);
        bytes.extend_from_slice(self.x25519_public.as_bytes());
    ...
    /// Import from bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        if bytes.len() != 32 + ML_KEM_768_CT_SIZE {
        ...
        Ok(Self {
            x25519_public: X25519PublicKey::from(x25519_bytes),
            ml_kem_ct: ml_kem_bytes,
        })
    }

    /// Get the total ciphertext size.
    pub fn size() -> usize {
        32 + ML_KEM_768_CT_SIZE
    }
}
...
impl X25519MlKem768 {
    ...
    pub fn generate_keypair() -> Result<(X25519MlKem768DecapsulationKey, X25519MlKem768EncapsulationKey)> {
    ...
    ) -> Result<(X25519MlKem768Ciphertext, X25519MlKem768SharedSecret)> {
        // X25519 key exchange
...
mod tests {
    ...
    fn test_hybrid_roundtrip() {
        let (dk, ek) = X25519MlKem768::generate_keypair().unwrap();
        let (ct, ss1) = X25519MlKem768::encapsulate(&ek).unwrap();
    ...
    fn test_hybrid_different_keypairs() {
        let (dk1, ek1) = X25519MlKem768::generate_keypair().unwrap();
        let (dk2, _ek2) = X25519MlKem768::generate_keypair().unwrap();

        let (ct, ss1) = X25519MlKem768::encapsulate(&ek1).unwrap();
    ...
    fn test_encapsulation_key_serialization() {
        let (_dk, ek) = X25519MlKem768::generate_keypair().unwrap();

        let bytes = ek.to_bytes();
        assert_eq!(bytes.len(), 32 + ML_KEM_768_EK_SIZE);
    ...
    fn test_ciphertext_serialization() {
        let (_dk, ek) = X25519MlKem768::generate_keypair().unwrap();
        let (ct, _ss) = X25519MlKem768::encapsulate(&ek).unwrap();
