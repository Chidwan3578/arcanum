...
fn generator_h() -> RistrettoPoint {
    ...
    RistrettoPoint::from_uniform_bytes(&hash_output)
}
...
impl PedersenCommitment {
    ...
    pub fn from_bytes(bytes: &[u8; 32]) -> Result<Self> {
        // from_slice in curve25519-dalek-ng returns CompressedRistretto directly
        // (panics if wrong size, but we have &[u8; 32] so size is guaranteed)
...
mod tests {
    ...
    fn test_commitment_zero() {
        ...
        let opening = PedersenOpening::from_scalar(DalekScalar::zero());
