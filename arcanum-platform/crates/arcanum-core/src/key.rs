...
impl<const N: usize> TryFrom<Vec<u8>> for SecretKey<N> {
    type Error = Error;
...
impl<const N: usize> TryFrom<Vec<u8>> for PublicKey<N> {
    type Error = Error;
...
///
/// The secret key is automatically zeroized when dropped via the contained SecretKey type.
#[derive(Clone)]
...
impl<const SK: usize, const PK: usize> KeyPair<SK, PK> {
    ...
    pub fn into_secret_key(self) -> SecretKey<SK> {
        self.secret_key
    }

    /// Consume and return both keys.
    pub fn into_parts(self) -> (SecretKey<SK>, PublicKey<PK>) {
...
impl fmt::Display for KeyUsage {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            KeyUsage::KeyExchange => write!(f, "keyExchange"),
            KeyUsage::WrapKey => write!(f, "wrapKey"),
            KeyUsage::DeriveKey => write!(f, "deriveKey"),
            KeyUsage::Authenticate => write!(f, "authenticate"),
        }
    }
}
...
pub enum KeyAlgorithm {
    ...
    EcdsaSecp256k1,
    /// Schnorr signatures with secp256k1 (BIP-340).
    SchnorrSecp256k1,
    ...
    // Post-Quantum
    /// ML-KEM-512 key encapsulation (FIPS 203, 128-bit security).
    MlKem512,
    /// ML-KEM-768 key encapsulation (FIPS 203, 192-bit security).
    MlKem768,
    /// ML-KEM-1024 key encapsulation (FIPS 203, 256-bit security).
    MlKem1024,
    /// ML-DSA-44 signature (FIPS 204, 128-bit security).
    MlDsa44,
    /// ML-DSA-65 signature (FIPS 204, 192-bit security).
    MlDsa65,
    /// ML-DSA-87 signature (FIPS 204, 256-bit security).
    MlDsa87,
    /// SLH-DSA-SHA2-128f signature (FIPS 205, fast variant).
    SlhDsaSha2_128f,
    /// SLH-DSA-SHA2-128s signature (FIPS 205, small variant).
    SlhDsaSha2_128s,
    /// SLH-DSA-SHA2-192f signature (FIPS 205, fast variant).
    SlhDsaSha2_192f,
    /// SLH-DSA-SHA2-192s signature (FIPS 205, small variant).
    SlhDsaSha2_192s,
    /// SLH-DSA-SHA2-256f signature (FIPS 205, fast variant).
    SlhDsaSha2_256f,
    /// SLH-DSA-SHA2-256s signature (FIPS 205, small variant).
    SlhDsaSha2_256s,
...
mod tests {
    ...
    fn test_secret_key_from_array() {
        let bytes = [42u8; 32];
        let key: SecretKey<32> = bytes.into();
        assert_eq!(key.as_bytes(), &bytes);
    }
    ...
    fn test_secret_key_try_from_slice() {
        let bytes = [42u8; 32];
        let key: SecretKey<32> = (&bytes[..]).try_into().unwrap();
        assert_eq!(key.as_bytes(), &bytes);

        // Wrong length should fail
        let short = [0u8; 16];
        let result: Result<SecretKey<32>> = (&short[..]).try_into();
        assert!(result.is_err());
    }

    #[test]
    fn test_secret_key_try_from_vec() {
        let vec = vec![42u8; 32];
        let key: SecretKey<32> = vec.try_into().unwrap();
        assert_eq!(key.as_bytes()[0], 42);

        // Wrong length should fail
        let short_vec = vec![0u8; 16];
        let result: Result<SecretKey<32>> = short_vec.try_into();
        assert!(result.is_err());
    }
    ...
    fn test_public_key_from_array() {
        let bytes = [42u8; 32];
        let key: PublicKey<32> = bytes.into();
        assert_eq!(key.as_bytes(), &bytes);
    }
    ...
    fn test_public_key_try_from_slice() {
        let bytes = [42u8; 32];
        let key: PublicKey<32> = (&bytes[..]).try_into().unwrap();
        assert_eq!(key.as_bytes(), &bytes);
    }

    #[test]
    fn test_public_key_try_from_vec() {
        let vec = vec![42u8; 32];
        let key: PublicKey<32> = vec.try_into().unwrap();
        assert_eq!(key.as_bytes()[0], 42);
    }
    ...
    fn test_key_usage_display() {
        ...
        assert_eq!(KeyUsage::Sign.to_string(), "sign");
        assert_eq!(KeyUsage::KeyExchange.to_string(), "keyExchange");
        assert_eq!(KeyUsage::WrapKey.to_string(), "wrapKey");
        assert_eq!(KeyUsage::DeriveKey.to_string(), "deriveKey");
        assert_eq!(KeyUsage::Authenticate.to_string(), "authenticate");
    }

    #[test]
    fn test_key_algorithm_display() {
        assert_eq!(KeyAlgorithm::Aes256.to_string(), "AES-256");
        assert_eq!(KeyAlgorithm::ChaCha20Poly1305.to_string(), "ChaCha20-Poly1305");
        assert_eq!(KeyAlgorithm::Ed25519.to_string(), "Ed25519");
        assert_eq!(KeyAlgorithm::X25519.to_string(), "X25519");
        assert_eq!(KeyAlgorithm::MlKem768.to_string(), "ML-KEM-768");
        assert_eq!(KeyAlgorithm::Custom("MyAlg".to_string()).to_string(), "MyAlg");
    }
}
